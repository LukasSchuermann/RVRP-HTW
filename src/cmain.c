   ...
   )
{
   ...
   /* include vehicle assignment branching and constraint handler*/
   /* include arc flow branching and branching data */
   if(veAssBranching){
       SCIP_CALL(SCIPincludeBranchruleVehicleAss(*scip, 50001));
       SCIP_CALL( SCIPincludeBranchruleArcFlow(*scip, 50000) );
   }else{
       SCIP_CALL(SCIPincludeBranchruleVehicleAss(*scip, 50000));
       SCIP_CALL( SCIPincludeBranchruleArcFlow(*scip, 50001) );
   }
   SCIP_CALL(SCIPincludeConshdlrVehicleAss(*scip));
...
//   SCIP_CALL( SCIPprintStatistics(scip, NULL) );
   ...
   if (INFEASIBILITY_RECOVERY && probdata->optionalCost > 0)
   {
      ...
      SCIP_CALL( setUpScip(&scipRecovery, veAssBranching));
